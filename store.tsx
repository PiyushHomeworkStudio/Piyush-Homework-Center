...
interface StoreContextType extends ExtendedAppState {
...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const dbChannel = supabase.channel('global-updates')
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (payload) => {
        ...
        setState(prev => {
          ...
          const isMe = prev.currentUser?.isAdmin ? OWNER_NUMBER : prev.currentUser?.id;
          
          // If the message is intended for me and I am NOT the sender
          if (newMessage.receiverId === isMe && newMessage.senderId !== isMe) {
            newUnreadCount += 1;
          }
    ...
    if (state.currentUser) {
      presenceChannel = supabase.channel('presence-system', {
      ...
      presenceChannel
        ...
        .on('presence', { event: 'join', key: state.currentUser.id }, () => {
          if (stateRef.current.currentUser) {
            supabase.from('profiles')
              .update({ last_seen: new Date().toISOString() })
              .eq('id', stateRef.current.currentUser.id)
              .then();
          }
        })
  ...
  const updateConfig = async (updates: any) => {
    ...
    if (error) console.error("Config Update Error:", error);
